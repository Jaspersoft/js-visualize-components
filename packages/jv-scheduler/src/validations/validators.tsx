...
// // @ts-ignore
// import globalConfig from 'jrs-ui/src/settings/globalConfiguration.settings';
// import moment from 'moment';
// import jrsConfigs from 'js-sdk/src/jrs.configs';
// import { IStepperState, IAlertErrors } from '../types/alertTypes';
// import { accessiblePermissionMask, ERROR_FIELDS } from '../constants/alertConstants';
// import { checkPermissionOnFolder } from '../services/alertServices';
//
...
//     const timeZone = jrsConfigs.userTimezone,
...
//     }
//     const folderUriError: {folderURI: string | undefined} = { folderURI: undefined };
//     try {
//         const validResourceProps = await checkPermissionOnFolder(folderUri)
//         if (accessiblePermissionMask.indexOf(validResourceProps.permissionMask) < 0) {
//             folderUriError.folderURI = 'error.report.alert.output.folder.notwriteable';
//         }
//     } catch (e: any) {
//         folderUriError.folderURI = e.responseJSON ? `error.${e.responseJSON?.errorCode}` : 'error.unexpected.occurred';
//     }
//     validatedFolderURIsCache[folderUri] = folderUriError;
//     return folderUriError
// }
//
// const validator = (propName:string, propVal: string, extraParams?: Pick<IStepperState, 'startType'|'outputTimeZone'>) => {
//     let AlertPropError;
//     switch (propName) {
//     case ERROR_FIELDS.NAME:
//         if (isFieldEmpty(propVal)) {
//             AlertPropError = 'error.alert.name';
//         } else if (propVal.length > 100) {
//             AlertPropError = 'error.alert.name.too.long';
//         }
//         break;
//     case ERROR_FIELDS.THRESHOLD:
//         if (isFieldEmpty(propVal)) {
//             AlertPropError = 'error.threshold';
//         }
//         break;
//     case ERROR_FIELDS.RECURRENCE:
//         if (isFieldEmpty(propVal)) {
//             AlertPropError = 'error.recurrence';
//         } else if (String(propVal).includes('.') || !Number.isInteger(Number(propVal)) || Number(propVal) <= 0) {
//             AlertPropError = 'error.recurrence.must.be.integer';
//         }
//         break;
//     case ERROR_FIELDS.START_DATE:
//         if (extraParams?.startType === 2 && isFieldEmpty(propVal)) {
//             AlertPropError = 'error.start.date';
//         } else if (isPastDate(propVal)) {
//             AlertPropError = 'error.past.date';
//
//         }
//         break;
//     case ERROR_FIELDS.EMAIL_ADDRESS:
//         if (isFieldEmpty(propVal)) {
//             AlertPropError = 'notifications.send.alert.to.helper.text';
//         } else if (!validateEmail(propName, propVal)) {
//             AlertPropError = 'error.email.address';
//         }
//         break;
//
//     case ERROR_FIELDS.EMAIL_SUBJECT:
//         if (isFieldEmpty(propVal)) {
//             AlertPropError = 'error.enter.subject';
//         } else if (propVal.length > 100) {
//             AlertPropError = 'error.subject.too.long';
//         }
//         break;
//     case ERROR_FIELDS.MESSAGE:
//         if (propVal.length > 2000) {
//             AlertPropError = 'error.message.too.long';
//         }
//         break;
//     case ERROR_FIELDS.FILE_NAME:
//         if (isFieldEmpty(propVal)) {
//             AlertPropError = 'error.file.name';
//         } else if (propVal.length > 200) {
//             AlertPropError = 'error.file.name.too.long';
//         } else if (!isValidFileName(propVal)) {
//             AlertPropError = 'error.invalid.file.name';
//         }
//         break;
//     case ERROR_FIELDS.OUTPUT_FORMAT:
//         if (!propVal?.length) {
//             AlertPropError = 'error.output.format';
//         }
//         break;
...
//             return { folderURI: 'error.report.alert.output.folder.resourceuri.format' }
//         }
...
//         break;
//     }
//     return { [propName]: AlertPropError };
// };
//
// export const stateValidator = (state:IAlertErrors | IStepperState) => {
//     const promises:Array<any> = []
//     const extraParams:{startType:number|undefined, outputTimeZone:string|undefined} = {
//         startType: undefined,
...
//         }
//         promises.push(validator(propertyName, (propertyValue as string), extraParams))
//     });
//
