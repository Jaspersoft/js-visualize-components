...
import { VisualizeFactory } from "../../index";
...
const defaultVisualizeUrl = "";

const createScriptInHeadAndWaitForLoad = (
  url: string,
  timeout: number = SCRIPT_LOAD_TIMEOUT,
): Promise<void> => {
  return new Promise<void>((resolve, reject) => {
    const timeoutHandle = setTimeout(() => {
      reject(
        `Failed to load visualizejs. Either error or timeout occurred. url: [${url}]`,
      );
    }, timeout);
...
export const globalVisualizejsProvider: VisualizejsProvider = () => {
  ...
  return localVisualizejs
    ? Promise.resolve(localVisualizejs)
    : getNoGlobalVizError();
};

export const visualizejsLoader = (
  url?: string,
  timeout?: number,
): VisualizejsProvider => {
