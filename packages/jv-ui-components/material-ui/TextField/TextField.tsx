...
export const TextField = forwardRef<HTMLDivElement, TextFieldProps>(
  (
    ...
    const { classes: inputLabelPropsClasses = {}, ...InputLabelPropsRest } =
      InputLabelProps;
    const { classes: inputPropsClasses = {}, ...InputPropsRest } = InputProps;
    const { classes: selectPropsClasses = {}, ...SelectPropsRest } =
      SelectProps;
    const { classes: helperTextPropsClasses = {}, ...HelperTextPropsRest } =
      FormHelperTextProps;
    const { className: errorTextPropsClassName = "", ...ErrorTextPropsRest } =
      FormErrorTextProps;

    const hasError = Boolean(error);
    const hasErrorText = hasError && typeof error === "string";
    const errorTextId =
      errorId ?? (hasError && id ? `${id}-error-text` : undefined);
    const helperTextId = helperText && id ? `${id}-helper-text` : undefined;

    const inputLabelProps = {
      classes: {
        root: `jv-mInput-label mui ${inputLabelPropsClasses?.root ?? ""}`,
    ...
    const inputAriaDescribedBy = [helperTextId, errorTextId].reduce(
      (acc, textId) => {
    ...
    const inputProps = select
      ...
      : {
          classes: {
            input: `jv-mInput-text mui ${inputPropsClasses?.input ?? ""}`,
            ...inputPropsClasses,
          },
    ...
    const selectProps = {
      classes: {
        select: `jv-mInput-select mui ${selectPropsClasses?.select ?? ""}`,
        ...selectPropsClasses,
      },
      ...(inputAriaDescribedBy
        ? { "aria-describedby": inputAriaDescribedBy }
    ...
    const helperTextProps = {
      classes: {
        root: `jv-mInput-helper mui ${helperTextPropsClasses?.root ?? ""}`,
        ...helperTextPropsClasses,
      },
      ...HelperTextPropsRest,
    };
    ...
    return (
      <div ref={ref} className={className} {...WrapperProps}>
        ...
        {hasErrorText && (
          <FormHelperText
            className={`jv-mInput-error mui ${errorTextPropsClassName}`}
            id={errorTextId}
            {...ErrorTextPropsRest}
          >
            {error}
