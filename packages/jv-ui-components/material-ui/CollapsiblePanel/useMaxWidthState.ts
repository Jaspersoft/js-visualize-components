...
export const generateInitialState = (
  collapsiblePanelMaxWidth: number,
...
const useCollapsiblePanelWidthOpenState = (
  callback: () => void,
  panelsState: PanelsState,
) => {
  const initialPanelsWidthHash = useMemo(
    () => getPanelsWidthOpenHash(panelsState),
    [panelsState],
  );
  useEffect(() => {
    callback();
...
const calcPanelMaxWidth = (panelMaxWidth: MaxWidth, tabsPanelWidth: number) =>
  getMaxWidth(panelMaxWidth) - tabsPanelWidth;

export const useMaxWidthState = (
  panelMaxWidth: MaxWidth,
  tabsPanelWidth: number,
  panelsState: PanelsState,
) => {
  const initialMaxWidth = useMemo(
    () => calcPanelMaxWidth(panelMaxWidth, tabsPanelWidth),
    [panelMaxWidth, tabsPanelWidth],
  );
  const [collapsiblePanelMaxWidth, setCollapsiblePanelMaxWidth] =
    useState(initialMaxWidth);

  useEffect(() => {
    setCollapsiblePanelMaxWidth(
      calcPanelMaxWidth(panelMaxWidth, tabsPanelWidth),
    );
  }, [panelMaxWidth, tabsPanelWidth]);
  ...
  useCollapsiblePanelWidthOpenState(() => {
    const newPanelMaxWidthState = generateInitialState(
      collapsiblePanelMaxWidth,
      panelsState,
    );
