...
export type SubPanelOpenStateDefinition = Required<
  Pick<SubPanelDefinition, "id" | "open">
>;
...
export type SubPanelHeightStateDefinition = Required<
  Pick<SubPanelDefinition, "id" | "height">
>;

export interface PanelSizeStateDefinition
  extends Pick<PanelDefinition, "id" | "width"> {
...
export type OnExpansionStateChangedCallback = (
  state: PanelOpenStateDefinition[],
) => void;

export type OnExpandCollapseClick = (
  props: PanelOrSubPanelActionProps,
...
export const DIRECTION_CLASS: {
  [key in Anchor]: {
    panelDirectionClass: string;
    panelsDirectionClass: string;
  };
} = {
  right: {
