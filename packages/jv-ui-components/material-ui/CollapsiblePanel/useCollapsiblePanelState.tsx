...
export const isPanelFullyExpanded = (
  panelIndex: number,
  state: PanelsState,
) => {
  return !state[panelIndex].subPanels.some((subPanel) => !subPanel.open);
...
export const getPanelSizeState = (
  state: PanelsState,
): PanelSizeStateDefinition[] => {
...
const generateSubPanelExpansionState = (
  state: PanelsState,
  panelIndex: number,
  subPanelsExpansionState: boolean[],
) => {
...
const getNewPanelState = (
  ...
  if (typeof open === "boolean") {
    subPanels = generateSubPanelExpansionState(
      state,
      panelIndex,
      subPanels.map(() => open),
    );
...
const generateInitialSubPanelsState = (
...
export type SetPanelState = (
  panelIndex: number,
  open?: boolean,
  width?: number,
) => void;
export type SetSubPanelState = (
  panelIndex: number,
  subPanelIndex: number,
  open?: boolean,
  height?: number,
...
type UseCollapsiblePanelState = [PanelsState, SetPanelState, SetSubPanelState];

export const useCollapsiblePanelState = (
  panels: PanelDefinition[],
): UseCollapsiblePanelState => {
  const initialPanelsState = useMemo(
    () => generateInitialPanelsState(panels),
    [panels],
  );
