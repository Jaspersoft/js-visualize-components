...
type CardProps = MuiCardProps & {
  ...
  cardHeaderProps?: Omit<MuiCardHeaderProps, "title"> & {
...
export const Message = forwardRef<HTMLDivElement, CardProps>(
  (
    ...
    return (
      <MuiCard
        ref={ref}
        className={`jv-mMessage ${TypeToClass[type]} ${className} mui`}
        elevation={elevation}
        {...rest}
      >
        {isCardHeader && (
        ...
        <MuiCardContent
          className={`jv-mMessage-body ${paddedClass} ${contentClassName} mui`}
          {...restCardContentProps}
        >
          {children}
        </MuiCardContent>
